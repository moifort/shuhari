...
final class ImportTabDismissTest: XCTestCase {
    ...
    func testClosingImportReturnsToPreviousTab() throws {
        ...
        // Start on Cuisine (the default tab).
        ...
        // Back on the tab bar: the previous category tab must still be selected,
        // and the "Importer" entry must never appear selected (its content is empty).
        try tabBar.waitOrFail()
