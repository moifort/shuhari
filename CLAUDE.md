...
- **Dish category** (`DISH_CATEGORY_VALUES`): `starter`, `main`, `dessert`, `soup`, `sauce`, `baking`, `drink`. Detected by the AI at import, fixed on the aggregate (never versioned); the array order IS the library's sort rank, denormalized via `categoryRank`.
