...
6. **Not at push time**: never touch `README.md` (updated on its own, when asked) or the changelogs — the changelog is written only during an [App Store release](docs/app-store-release.md), in the style of [changelog-best-practices.md](docs/changelog-best-practices.md).
...
- **A recipe wears tags, not its type** (`Recipe.tags: { label, icon? }[]`): free-text labels with an optional pictogram from a closed set (`TAG_ICON_VALUES`). A library row shows the **icons only**, the recipe sheet icon + label, where the type badge used to be — `Recipe.type` still shapes the content and picks the tab, it is just never a badge. A `thermomix` recipe is born tagged `"Thermomix"`; retagging goes through `updateRecipe` (full replacement, eight at most), creating no version and redating nothing — [docs/business-rules.md](docs/business-rules.md#tags--what-a-recipe-is-filed-under).
