...
// A transaction we can act on: signed by Apple, naming a cook, and granting time
...
export namespace EntitlementUseCase {
  ...
  // names *this* cook, and only then records the Premium.
  //
  // The account-token check is what stops a signed transaction from being
  // replayed onto someone else's account: the token is derived from the cook's
  ...
  //
  // A notification for a cook we have never recorded is acknowledged and dropped:
