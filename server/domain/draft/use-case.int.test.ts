...
describe('DraftUseCase.forTrial', () => {
  ...
  test('aligns tmxSteps with the steps for a tmx recipe, [] for a plat recipe', async () => {
    ...
    const tmx = await RecipeCommand.importRecipe(userId, recipeInput({ type: 'tmx' }))
    ...
    const plat = await RecipeCommand.importRecipe(userId, recipeInput())
...
describe('DraftUseCase.accept', () => {
  test('appends version n+1 from the passed draft and marks it to test', async () => {
    const recipe = await RecipeCommand.importRecipe(userId, recipeInput())
