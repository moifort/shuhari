#!/usr/bin/env bash
#
# bootstrap.sh — one-shot local provisioning of the Shuhari Firebase stack.
#
# Run this ONCE on a clean machine. Subsequent deploys go through GitHub
# Actions which talks to the same Terraform state stored in GCS.
#
...
EOF
  ...
  ok "State migrated to gs://${TFSTATE_BUCKET}/shuhari"
...
cat <<EOF
  ...
  Terraform state      : gs://${TFSTATE_BUCKET}/shuhari
  Admin token (sensitive, store in GitHub secret ADMIN_TOKEN):
                         ${ADMIN_TOKEN}

  Next steps
  ----------
  1. Commit infra/backend.tf so the deploy workflow uses the same remote
     state.
  2. Add the following GitHub secrets / variables:
       Secrets : GCP_BILLING_ACCOUNT, GCP_WIF_PROVIDER, GCP_DEPLOY_SA,
                 ...
                 APPLE_PRIVATE_KEY_P8, GOOGLE_API_KEY,
